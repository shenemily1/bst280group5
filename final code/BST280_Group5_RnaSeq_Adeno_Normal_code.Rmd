...
ggplot(cluster_df, aes(x = cluster)) +
  ...
  theme_bw() +
  labs(title = "Sample Count per K-means Cluster (k = 6), padj < 0.1",
...
ggplot(df_pca, aes(PC1, PC2, color = cluster)) +
  geom_point(size=3) +
  theme_bw() +
  labs(title = "K-means Clusters Shown on PCA Space, padj < 0.1")
```
...
ggplot(cluster_df, aes(x = cluster)) +
  geom_bar(fill = "steelblue") +
  theme_bw() +
  labs(title = "Sample Count per K-means Cluster (k = 4), padj < 0.05 & log2 > 2",
       x = "Cluster", y = "Number of Samples")
...
Both methods revealed a system-wide shift in gene expression between tumor (TCGA-LUAD) and normal (GTEx lung) samples. However, this pattern likely reflects systematic differences between data sources—such as technical, procedural, or cohort-level variation—rather than purely biological differences. Because the dataset indicator (TCGA vs. GTEx) is perfectly colinear with case status (tumor vs. normal), batch effect and phenotype cannot be disentangled. For this reason, both DESeq2 and limma-voom were run not using the batch-corrected expression values. Correcting for dataset batch would have removed the tumor–normal contrast entirely.

PCA restricted to DEGs also showed separation similar to the global PCA, suggesting that most differential expression signal reflects dataset-specific variation rather than tumor-specific transcriptional changes. When we restricted the sample to a more stringent threshold, the PCA separation became cleaner, suggesting the genes with both small p values and high log fold change may be driving the true biological signals.
...
6. Deep Dive Into a Key Gene: EGFR
6.1 Functional Overview and Clinical Relevance
To understand the mechanistic drivers of Lung Adenocarcinoma (LUAD), we performed a high-resolution analysis of EGFR (Epidermal Growth Factor Receptor). According to NCBI Gene documentation, EGFR is a member of the protein kinase superfamily and encodes a transmembrane glycoprotein. The receptor is activated by binding to epidermal growth factor family members, inducing dimerization and autophosphorylation that ultimately triggers cell proliferation. Pathologically, mutations in EGFR are classically associated with lung cancer development and the orchestration of inflammatory signaling.

6.2 Genomic Architecture and Regulatory Landscapes
We utilized the UCSC Genome Browser to map the physical regulatory landscape of the EGFR locus on Chromosome 7p11.2. The locus exhibits significant structural complexity, featuring 32 exons and multiple curated RefSeq and GENCODE isoforms. The Layered H3K27Ac track from ENCODE reveals high-intensity enrichment at the canonical promoter and upstream regions. Since H3K27Ac is a hallmark of active promoters and enhancers, these peaks represent accessible "genomic gates" where transcription factors can readily bind. Furthermore, the Cons 100 Verts track highlights that these regulatory sequences are highly conserved across vertebrates, underscoring their critical biological function.

6.3 PANDA Regulatory Rewiring Analysis
Our PANDA network modeling provides a computational explanation for the aberrant activation of EGFR in the tumor state. By comparing LUAD (Cancer) and GTEx (Normal) networks, we identified a profound reorganization of the transcription factors targeting the EGFR gene. Transcription factors such as ZNF334, FOXF1, and ARID3A showed the most significant increase in targeting force in LUAD. The differential targeting data identifies ZNF334 as a top positive regulator gaining influence in the cancer network, while several homeostatic transcription factors exhibited a loss of targeting force, suggesting a breakdown of normal regulatory constraints. Notably, FOXF1, which gains targeting force toward EGFR, is also significantly enriched in the KEGG Ubiquitin Mediated Proteolysis pathway. This suggests a coordinated oncogenic program where rewired transcription factors simultaneously increase EGFR production and potentially alter its degradation pathways to maintain sustained growth signaling.

6.4 Conclusion
The synergy between the physical landscape of UCSC H3K27Ac peaks and the network dynamics of PANDA targeting force confirms that EGFR acts as a central hub for transcriptional reprogramming in LUAD. The cancer-specific hijacking of these conserved regulatory elements by rewired transcription factors provides a robust mechanistic explanation for the elevated EGFR activity observed in patient samples, linking epigenetic accessibility directly to oncogenic signaling.
