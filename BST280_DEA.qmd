...
```{r}
de_genes <- rownames(res)[ which(res$padj < 0.05 & !is.na(res$padj)) ]
length(de_genes)
...
```{r, warning=FALSE, message=FALSE}
logcpm_bc <- assayData(filtered_eset)$norm_logtpm  # batch-corrected log-TPM/log-CPM
coldata <- pData(filtered_eset)

# same filtered samples 
# vars_for_design <- c("sex", "age_bin", "group")
# keep <- complete.cases(coldata[, vars_for_design])
logcpm_bc_filt <- logcpm_bc[, keep]
# coldata_filt   <- coldata[keep, ]

library(limma)

design_limma <- model.matrix(~ sex + age_bin + group, data = coldata_filt)
#design_limma <- model.matrix(~ group, data = coldata_filt)

```

```{r}
fit_limma <- lmFit(logcpm_bc_filt, design_limma)
...
mat <- logcpm_bc_filt[rownames(res_limma)[1:20],]
